...
**REQUIRED**: Before any other action, verify that the `.specify/` directory exists in the project root.

- If `.specify/` does **NOT** exist: Output the following error and **abort**:
  ```
  Error: .specify/ workspace not found. Run /cc-spec-kit-core:init to initialize.
  ```
- If `.specify/` exists: proceed to the next step.
